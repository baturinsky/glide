...
void main() {
  //v_uv = uv[imod(gl_VertexID,6)];
  //vec4 position = vec4(quad[imod(gl_VertexID,6)] * 200., 0.);
  // vec4(quad[gl_VertexID] * 0.8, 0.);
  ...
  //v_color = vec4(uv[gl_VertexID % 6], 0., 1.);
  //v_color = vec4(normalize(vec3(rand(float(cubeID*2)), rand(float(cubeID*2)+0.1), rand(float(cubeID*2) + 0.2) * 2.)), 1.);

  //v_color = vec4(sideID%2, sideID/2%2, sideID/4%2, 1);  
